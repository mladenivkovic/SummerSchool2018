...
void diffusion(data::Field &U, data::Field &S)
{
    ...
    {
    #pragma acc data present (S, U, x_old)
    ...
    {
        int i = nx - 1;
        #pragma acc data present (S, U, x_old, bndE)
    ...
    {
        int i = 0;
        #pragma acc data present (S, U, x_old, bndW)
        #pragma acc parallel loop async(1)
        for (int j = 1; j < jend; j++)
        {
            S(i,j) = -(4. + alpha) * U(i,j)
    ...
    // the north boundary (plus NE and NW corners)
    {
        ...
        // north boundary
        #pragma acc data present (S, U, x_old, bndN)
        #pragma acc parallel loop async(1)
        for (int i = 1; i < iend; i++)
        {
            S(i,j) = -(4. + alpha) * U(i,j)
    ...
    // the south boundary
    {
        ...
        // south boundary
        #pragma acc data present (S, U, x_old, bndS)
        #pragma acc parallel loop async(1)
        for (int i = 1; i < iend; i++)
        {
            S(i,j) = -(4. + alpha) * U(i,j)
